...
from .analysis.non_verbal import NonVerbalAggregator
...
class WebSocketHandler:
    ...
    def __init__(self, default_context_cache: ServiceContext, config=None):
        ...
        self.client_aggregators: Dict[str, NonVerbalAggregator] = {}

        # Message handlers mapping
        self._message_handlers = self._init_message_handlers()

        # Interview Manager
        if config and hasattr(config, "rag_config"):
            self.interview_manager = InterviewManager(rag_config=config.rag_config)
        else:
    ...
    async def handle_new_connection(
        ...
        try:
            ...
            logger.warning(f"Client {client_uid} disconnected during initialization (likely double-mount)")
            await self._cleanup_failed_connection(client_uid)
        except Exception as e:
            # Check for "going away" message in exception string if it's wrapped
            if "1001" in str(e) or "going away" in str(e):
                 logger.warning(f"Client {client_uid} disconnected during initialization (benign): {e}")
                 await self._cleanup_failed_connection(client_uid)
                 return
    ...
    async def _store_client_data(
        ...
        self.client_aggregators[client_uid] = NonVerbalAggregator(client_uid)
    ...
    async def _init_service_context(
        ...
        session_service_context = ServiceContext()
        session_service_context.interview_manager = self.interview_manager
    ...
    async def handle_disconnect(self, client_uid: str) -> None:
        ...
        self.client_aggregators.pop(client_uid, None)
        ...
        if group:
            ...
            try:
                await websocket.send_text(
                 ...
                 # RuntimeError can happen if connection is closed ("Connection is already closed")
                 logger.debug(f"Failed to send group update to {client_uid} (connection closed)")
        else:
            await websocket.send_text(
                json.dumps(
                    {
    ...
    async def _handle_init_interview_session(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """
        Handle initialization of a new interview session.
        Payload should contain 'jd' and 'resume' text.
        """
        logger.info(f"Initializing interview session for {client_uid}")
        ...
        # 1. Create Session
        session = self.interview_manager.create_session(client_uid, jd_text, resume_text, style=style)

        # 1.5. Prepare Session (RAG Analysis - Async)
        # This analyzes the resume and generates questions.
        await websocket.send_text(json.dumps({"type": "status", "message": "Analyzing resume..."}))
        await self.interview_manager.prepare_session(client_uid)

        # 2. Generate Dynamic System Prompt
        system_prompt = self.interview_manager.generate_system_prompt(session)

        # 3. Update ServiceContext with new Persona
        # We clone the default config but override the persona prompt
        context = self.client_contexts.get(client_uid)
        if not context:
            # Should already be initialized in handle_new_connection, but safety check
            logger.warning(f"Context not found for {client_uid}, re-initializing")
            context = await self._init_service_context(websocket.send_text, client_uid)
            self.client_contexts[client_uid] = context

        new_config = context.config.model_copy(deep=True)
        new_config.character_config.persona_prompt = system_prompt
        ...
        await websocket.send_text(
            json.dumps({
                ...
                "status": "ready",
                "client_uid": client_uid,
                "message": "AI Interviewer is ready with your context."
            })
        )

        # 5. Trigger First Greeting (Optional)
        # We can ask the AI to introduce itself based on the new prompt
        # We must use process_single_conversation to handle the async generator stream propery
        # and run it as a background task to avoid blocking the websocket loop.
        task = asyncio.create_task(
            process_single_conversation(
                context=context,
                websocket_send=websocket.send_text,
                client_uid=client_uid,
                user_input="면접관으로서 첫 인사를 건네고 면접을 시작해주세요.",
                metadata={
    ...
    async def _handle_update_interview_phase(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """
        Handle transition to a new interview phase.
        Payload should contain 'phase' (e.g., 'technical', 'behavioral').
        """
        new_phase = data.get("phase")
        if not new_phase:
            logger.warning(f"Phase update requested for {client_uid} without 'phase' data.")
            return

        logger.info(f"Updating interview phase for {client_uid} to {new_phase}")

        session = self.interview_manager.get_session(client_uid)
        if not session:
            logger.warning(f"No active session found for {client_uid} to update phase.")
            return

        # 1. Update Session State
        session.current_phase = new_phase

        # 2. Generate New System Prompt (Memory Safe)
        # We do NOT use load_from_config here to preserve chat history.
        new_system_prompt = self.interview_manager.generate_system_prompt(session)

        # 3. Hot-Swap System Prompt in Agent
        context = self.client_contexts.get(client_uid)
        if context and context.agent_engine:
            context.agent_engine.set_system(new_system_prompt)
            logger.info(f"System prompt hot-swapped for {client_uid}: {new_phase}")

            # Get guide for the new phase
            guide_text = self.interview_manager.get_phase_guide(new_phase)
            ...
            await websocket.send_text(
                json.dumps({
                    ...
                    "phase": new_phase,
                    "guide": guide_text,
                    "message": f"Interview phase changed into {new_phase}."
                })
            )

    async def _handle_behavior_data(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Handle incoming behavior data (gaze, pose)"""
        if client_uid not in self.client_aggregators:
            return
        ...
        try:
            model = NonVerbalData(**payload)
            aggregator = self.client_aggregators[client_uid]
            feedback = aggregator.add_data(model)
